...
define( 'WOOCOMMERCE_THANKS_REDIRECT_PLUGIN_VERSION', '3.1' );
	...
	add_action(
		...
		function ( $order_id ) {
			...
			if ( isset( $wctr_global ) && strtolower( $wctr_global ) === 'yes' ) {
				...
				$order_string = "&order_id=$order_id";
                ...
                foreach ($items as $key => $this_product) { // phpcs:ignore
...
function wc_thanks_redirect_install_admin_notice() {
	...
	echo '<p>' . sanitize_html_class( __( 'Thanks Redirect for WooCommerce requires active WooCommerce Installation, please install and activate WooCommerce plugin!', 'wc-thanks-redirect' ) ) . '</p>';
	echo '</div>';
}

function wc_thanks_redirect_multisite_admin_notice() {
	echo '<div class="notice notice-error">';
	echo '<p>' . sanitize_html_class( __( 'Thanks Redirect for WooCommerce is not designed for Multisite, you may need to buy this short plugin. <a target="_blank" href="https://bit.ly/2RwaIQB">Thanks Redirect for WooCommerce PRO</a>!', 'wc-thanks-redirect' ) ) . '</p>';
	echo '</div>';
}
...
function wc_thanks_redirect_action_links( $links ) {
	$links = array_merge(
		array(
			...
			'<a href="' . esc_url( site_url() . '/wp-admin/admin.php?page=wctr-docs' ) . '">' . __(
				'Documentation',
				'wc-thanks-redirect-pro'
			) . '</a>',
...
function wc_thanks_redirect_short_code_order_details( $atts ) {
    ...
    $order_id = !empty($_GET['order_id']) ? wp_kses_post($_GET['order_id']) : 0; // phpcs:ignore
		  ...
		  <section class="woocommerce-order-details">
			...
			<ul class="woocommerce-order-overview woocommerce-thankyou-order-details order_details">
			  ...
			  <li class="woocommerce-order-overview__order order">
				...
				?>
				: <strong><?php echo wp_kses_post( wp_date( $wctr_date_format, strtotime( $order->date_created ) ) ); ?></strong>
			  </li>
			  ...
			  <li class="woocommerce-order-overview__order order">
				<?php esc_html_e( 'Payment Method', 'wc-thanks-redirect' ); ?>: <strong><?php echo wp_kses_post( $order->payment_method_title ); ?></strong>
			  </li>
				...
				?>
								...
								<tfoot>
									...
									foreach ( $order->get_order_item_totals() as $key => $total ) {
										...
										<td><?php ( 'payment_method' === $key ) ? esc_html_e( $total['value'] ) : wp_kses_post( $total['value'] ); ?></td>
										</tr>
										<?php
									}
									?>
...
function wc_thanks_redirect_wctr_docs() {
	...
	$default_tab = 'docs';
    $tab = !empty($_GET['tab']) ? sanitize_text_field($_GET['tab']) : $default_tab; // phpcs:ignore
	?>

<div class="wrap">
	<!-- Here are our tabs -->
	<nav class="nav-tab-wrapper">
	  <a href="<?php echo esc_url( site_url() ) . '/wp-admin/admin.php?page=wc-settings&tab=products&section=wctr'; ?>" class="nav-tab 
						  <?php
							if ( $tab === null ) :
								?>
			nav-tab-active
							...
							?>
	 ">
			<?php esc_html_e( 'Thank You Page Settings', 'wc-thanks-redirect-pro' ); ?>
	  </a>
	  <a href="?page=wctr-docs&tab=docs" class="nav-tab 
	  <?php
		if ( $tab === 'docs' ) :
			?>
			nav-tab-active
		...
		?>
	 ">
			<?php esc_html_e( 'Documentation', 'wc-thanks-redirect-pro' ); ?>
	  </a>
	</nav>
	...
	<div class="tab-content">
		<?php
		if ( 'docs' === $tab ) {
			file_get_contents( WOOCOMMERCE_THANKS_REDIRECT_PLUGIN_DIR_PATH . 'readme.html' );
		}
		?>
	</div>
  </div>
